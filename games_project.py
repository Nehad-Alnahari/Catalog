...
APPLICATION_NAME = 'Games Menu Web client 2'
...
def gconnect():
    ...
    # See if a user exists, if it doesn't make a new user
...
# function of getting user information
...
def newCategory():
    ...
    if request.method == 'POST':
        ...
        flash('Category %s is added Successfully' % newCategory.name)
        session.commit()
...
def editCategory(category_id):
    ...
    if request.method == 'POST':
        if request.form['name']:
            ...
            flash('Category is edited Successfully %s'
                  % editedCategory.name)
...
def deleteCategory(category_id):
    ...
    if request.method == 'POST':
        ...
        flash('%s is deleted Successfully' % categoryToDelete.name)
        session.commit()
...
def newMenuItem(category_id):
    ...
    if request.method == 'POST':
        ...
        session.add(newItem)
        session.commit()
        flash('New Game %s is created Successfully' % newItem.name)
        #  back to the menu when finish deleting
...
def editMenuItem(category_id, menu_id):
    # edit game only if the user in login_session
    if 'username' not in login_session:
        return redirect('/login')
    # query to get item from the dtabase
    editedItem = session.query(Game).filter_by(id=menu_id).one()
    # query to get category from the database
    category = session.query(Category).filter_by(id=category_id).one()
    # Authorize editing, only menu creator can edit the game
    user = getUserInfo(login_session['user_id'])
    creator = getUserInfo(editedItem.user_id)
    if creator.id != user.id:
        flash('You cannot edit this game')
        return redirect(url_for('showCategories'))
    # getting the game information from the user
    if request.method == 'POST':
        if request.form['name']:
            editedItem.name = request.form['name']
        if request.form['description']:
            editedItem.file_link = request.form['description']
        if request.form['picture']:
            editedItem.picture = request.form['picture']
        if request.form['platform']:
            editedItem.author = request.form['platform']
        if request.form['developer']:
            editedItem.yearOfEmission = request.form['developer']
        if request.form['publisher']:
            editedItem.numOfPage = request.form['publisher']
        if request.form['producer']:
            editedItem.numOfPage = request.form['producer']
        if request.form['director']:
            editedItem.numOfPage = request.form['director']
        session.add(editedItem)
        session.commit()
        flash('Game is edited Successfully')
        # back to the menu after finishing deleting
        return redirect(url_for('showMenu', category_id=category_id))
    else:
...
def deleteMenuItem(category_id, menu_id):
    # delete game only if the user in login_session
    if 'username' not in login_session:
        return redirect('/login')
    # query to get category from the database
    category = session.query(Category).filter_by(id=category_id).one()
    # query to get item from the dtabase
    itemToDelete = session.query(Game).filter_by(id=menu_id).one()
    # Authorize editing, only menu creator can edit the game
    user = getUserInfo(login_session['user_id'])
    creator = getUserInfo(itemToDelete.user_id)
    if creator.id != user.id:
        flash('You cannot delete this game')
        return redirect(url_for('showCategories'))

    if request.method == 'POST':
        session.delete(itemToDelete)
        session.commit()
        flash('Game is deleted Successfully')
        # back to the menu after finishing deleting
        return redirect(url_for('showMenu', category_id=category_id))
    else:
...
def menuItemJSON(category_id, menu_id):
    Menu_Item = session.query(Game).filter_by(id=menu_id).one()
